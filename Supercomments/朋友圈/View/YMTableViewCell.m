...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        _userHeaderImage = [[UIImageView alloc] initWithFrame:CGRectMake(14, 16, 36*WIDTH_SCALE, 36*WIDTH_SCALE)];
        _userHeaderImage.backgroundColor = [UIColor clearColor];
        CALayer *layer = [_userHeaderImage layer];
        [layer setMasksToBounds:YES];
        [layer setCornerRadius:18*WIDTH_SCALE];
        [self.contentView addSubview:_userHeaderImage];
        
        _userNameLbl = [[UILabel alloc] initWithFrame:CGRectMake(14*WIDTH_SCALE+32*WIDTH_SCALE+14*WIDTH_SCALE, 14*HEIGHT_SCALE, 180*WIDTH_SCALE, 14*HEIGHT_SCALE)];
        _userNameLbl.textAlignment = NSTextAlignmentLeft;
        _userNameLbl.font = [UIFont systemFontOfSize:14*FX];
        _userNameLbl.textColor = [UIColor wjColorFloat:@"CDCDC7"];
        [self.contentView addSubview:_userNameLbl];
        
        
        
        _userIntroLbl = [[UILabel alloc] initWithFrame:CGRectMake(14*WIDTH_SCALE+32*WIDTH_SCALE+14*WIDTH_SCALE, 14*HEIGHT_SCALE+20*HEIGHT_SCALE, 80*WIDTH_SCALE, 11*HEIGHT_SCALE)];
        _userIntroLbl.numberOfLines = 1;
        _userIntroLbl.font = [UIFont systemFontOfSize:11.0*FX];
        _userIntroLbl.textColor = [UIColor wjColorFloat:@"CDCDC7"];
...
#pragma mark - ///// //最下方回复部分
    ...
    for (int i = 0; i < ymData.replyDataSource.count; i ++ ) {
        ...
        WFTextView *_ilcoreText = [[WFTextView alloc] initWithFrame:CGRectMake(offSet_X,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh  + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance + ymData.favourHeight , screenWidth - offSet_X * 2, 0)];
        
        if (i == 0) {
            backView_Y = TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh  + (ymData.islessLimit?0:30);
        }
        ...
        _ilcoreText.frame = CGRectMake(offSet_X,TableHeader + 10 + ShowImage_H + (ShowImage_H + 10)*(scale_Y/3) + origin_Y + hhhh  + (ymData.islessLimit?0:30) + balanceHeight + kReplyBtnDistance  + (ymData.favourHeight == 0?0:kReply_FavourDistance), screenWidth - offSet_X * 2, [_ilcoreText getTextHeight]);
